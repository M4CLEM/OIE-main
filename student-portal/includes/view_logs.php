...
class viewStudentLogs{
    ...
    public function loadStudentInfo($conn, $studentNum){

        $student_details_query = mysqli_query($conn, "SELECT * FROM studentinfo WHERE studentID='$studentNum'");
        $str = " ";
        ...
        if (mysqli_num_rows($student_details_query) > 0) {
            while ($row = mysqli_fetch_array($student_details_query)) {
                ...
                $str .= "
                        <h3 class='card-text'>$student_fullName</h3> <br>
    ...
    function loadStudentLogs($conn, $studentNumber, $dateFrom = null, $dateTo = null) {
