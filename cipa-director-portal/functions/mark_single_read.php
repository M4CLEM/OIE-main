<?php
    ...
    if (!isset($_POST['notification_id']) || !isset($_POST['employeeNumber'])) {
        echo json_encode([
            'status' => 'error',
            'message' => 'Missing parameters: notification_id or employeeNumber.'
        ]);
        exit;
    }
    ...
    if (empty($employeeNumber) || $reminderID <= 0) {
        echo json_encode([
            'status' => 'error',
